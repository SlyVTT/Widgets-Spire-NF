...
void WindowWidget::Logic( void )
{
    ...
    if (nbpopupvisible==0)
    {
        if (is_activated)
        {
            ...
            if (!movemode && !resizemode) // This is to improve movement speed (don't run childrens' logic during move and resize)
            {
                for (auto& c : children )
                    c->Logic( );
            }
...
void WindowWidget::Render( void )
{
    ...
    if (is_visible)
    {
        ...
        if (movemode || resizemode) // This is to improve movement speed (don't draw children during move and resize)
        {
